...
class FunctionBase:
    ...
    def chain_rule(cls, ctx, inputs, d_output):
        ...
        ls = []
        if not isinstance(derivs, (list, tuple)):
            derivs = [derivs]
        for var, deriv in zip(inputs, derivs):
            if is_constant(var):
                continue
            else:
                ls.append(VariableWithDeriv(var, deriv))
...
def backpropagate(final_variable_with_deriv):
    ...
    """
    # 0. Create queue of variables & derivatives
    queue_vars = []
    queue_vars.append(final_variable_with_deriv)

    # 1. While the queue is not empty...
    while queue_vars:

        # 1. Pull variables & derivative from queue
        item = queue_vars.pop(0)
        var, deriv = item.variable, item.deriv

        # 1. a) If variable is leaf, add its final derivative and loop to 1.
        if is_leaf(var):
            var._add_deriv(deriv)

        else:
            queue_vars.extend(var.history.backprop_step(deriv))